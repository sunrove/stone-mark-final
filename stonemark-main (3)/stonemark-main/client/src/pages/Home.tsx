...
import { Phone, ChevronDown, Star, ArrowRight, Shield, Clock, Award, DollarSign, Wrench, Users } from "lucide-react";
...
function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden" style={{ background: "#1C1F26" }}>
      <div className="absolute inset-0 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${HERO_IMAGE})` }} />
      <div className="absolute inset-0 bg-gradient-to-r from-[#1C1F26]/96 via-[#1C1F26]/82 to-[#1C1F26]/35" />
      <div className="absolute bottom-0 left-0 right-0 h-24 bg-[#3A3F4A]" style={{ clipPath: "polygon(0 60%, 100% 0%, 100% 100%, 0% 100%)" }} />
      ...
      <div className="container relative z-10 pt-24 pb-32">
        <div className="max-w-3xl">
          ...
          <h1
            className={`font-['Barlow_Condensed'] text-white leading-[0.92] mb-6 transition-all duration-700 delay-100 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          ...
          <p className={`font-['DM_Sans'] text-white/75 text-lg md:text-xl max-w-xl mb-8 leading-relaxed transition-all duration-700 delay-200 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
            Middle Tennessee's trusted roofing contractor with <strong className="text-white">45+ years of experience</strong>. GAF Certified, OSHA certified, and backed by a <strong className="text-white">5-star reputation</strong>.
          </p>
...
function StatsBar() {
  ...
  const stats = [
    ...
    { value: 22, suffix: "+", label: "Google Reviews" },
  ];
  return (
    <section id="stats" ref={ref} className="relative bg-[#3A3F4A] border-y border-[#F97316]/30" style={{ background: "linear-gradient(135deg, #1C1F26 0%, #22252e 100%)" }}>
          ...
          {stats.map((s, i) => {
            ...
            return (
              <div key={i} className="flex flex-col items-center text-center px-6 py-6 md:py-8 flex-1 min-w-[130px]">
                <div className="font-['Barlow_Condensed'] text-[#F97316] leading-none mb-1" style={{ fontSize: "clamp(2.5rem, 5vw, 4rem)", fontWeight: 900 }}>
                  {s.prefix}{count}{s.suffix}
                </div>
                <div className="font-['Barlow_Condensed'] text-white/60 uppercase tracking-widest text-xs font-semibold">{s.label}</div>
              </div>
            );
          })}
        </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10">
            {["GAF Certified Contractor", "OSHA Certified", "Licensed & Bonded", "A+ BBB Rating", "Middle TN Home Builders Award"].map((badge) => (
              <div key={badge} className="flex items-center gap-2">
                ...
                <span className="font-['DM_Sans'] text-white/60 text-xs uppercase tracking-widest">{badge}</span>
              </div>
            ))}
          </div>
        </div>
...
function ServicesOverview() {
  ...
  return (
    <section className="py-24 bg-[#3A3F4A]" ref={ref}>
      <div className="container">
        <div className={`mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          <div>
            ...
            <h2 className="font-['Barlow_Condensed'] text-white leading-none" style={{ fontSize: "clamp(2.5rem, 5vw, 4.5rem)", fontWeight: 900 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {serviceCards.map((s, i) => (
            ...
            >
              ...
              <div className="p-5">
                <h3 className="font-['Barlow_Condensed'] text-white text-lg uppercase tracking-wide mb-2 leading-tight" style={{ fontWeight: 800 }}>{s.title}</h3>
                <p className="font-['DM_Sans'] text-white/60 text-sm leading-relaxed mb-4">{s.desc}</p>
        ...
        <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
          {[
            ...
            <div key={s.title} className={`service-card p-6 flex items-start gap-4 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ transitionDelay: `${(i + 4) * 80}ms` }}>
              ...
              <div>
                <h3 className="font-['Barlow_Condensed'] text-white text-lg uppercase tracking-wide mb-1" style={{ fontWeight: 800 }}>{s.title}</h3>
                <p className="font-['DM_Sans'] text-white/60 text-sm leading-relaxed">{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
...
function WhyUsPreview() {
  const { ref, visible } = useVisible(0.1);
  return (
    <section className="py-24 relative overflow-hidden" style={{ background: "linear-gradient(135deg, #22252e 0%, #1C1F26 100%)" }} ref={ref}>
      ...
      <div className="container relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {highlights.map((item, i) => {
            ...
            return (
              <div key={item.title} className={`p-5 border border-white/10 bg-[#3A3F4A]/60 hover:border-[#F97316]/40 transition-all duration-300 group ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: `${i * 80}ms` }}>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="font-['Barlow_Condensed'] text-white text-base uppercase tracking-wide mb-1 leading-tight" style={{ fontWeight: 700 }}>{item.title}</h4>
                    <p className="font-['DM_Sans'] text-white/55 text-sm leading-relaxed">{item.desc}</p>
                  </div>
                </div>
              </div>
            );
    ...
    <section className="py-24 bg-[#3A3F4A]" ref={ref}>
      <div className="container">
        <div className={`mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          ...
          <div className="flex items-center gap-4 bg-[#4A5060] border border-white/10 px-6 py-4 self-start">
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
          {featuredReviews.map((r, i) => (
            <div key={i} className={`bg-[#4A5060] border border-white/10 p-6 hover:border-[#F97316]/40 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ transitionDelay: `${i * 80}ms` }}>
